...
#   2. Clones (or fast-forwards) the ds4 fork at tag v0.2 into $DS4_SRC_DIR.
...
#      $DS4_PORT (lossless; yield-quench + kv-gate ride the v0.2 defaults).
...
# Pin (updated 2026-07-15): the Entrpi/ds4 fork release tag v0.2 — the
...
start_server() {
    ...
    if [[ "$WITH_DSPARK" -eq 1 ]]; then
        ...
        log "Starting ds4-server with DSpark speculative decode (yield-quench + kv-gate ride the v0.2 defaults)."
